...
public final class OtterCommand {
    ...
    private static int runHelp(CommandSourceStack source) {
        ...
        sendLine(source, "/pay <player> <amount> — send coins to another player (atomic; clear feedback on insufficient funds)");
        sendLine(source, "/money set|add|take <player> <amount> — admin balance ops (gamemaster / OP-band only)");
        sendLine(source, "All balance changes are logged to config/otters_civ_revived/transactions.log (append-only CSV).");
